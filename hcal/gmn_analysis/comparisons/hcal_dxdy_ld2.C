		    ...
		    const char *outputfilename="hcal_dxdy_ld2.root" )
{
  ...
  while( C->GetEntry(elist->GetEntry(nevent++)) ){
	...
	pow( (yHCAL-yexpect_HCAL - dy0_n)/dysigma_n, 2 ) <= pow(1.5,2); 
      ...
      //**** fiducial cut ****
      bool fidu_cut_p, fidu_cut_n, HCAL_active;
      double deltax_shift;
      HCAL_active = yHCAL>-0.75 && yHCAL<0.75 && xHCAL>-2.015 && xHCAL<1.285;
      if(SBSM==30){
	deltax_shift=0.65;
	fidu_cut_p = (yexpect_HCAL-0.26)>-.75 && (yexpect_HCAL+0.26)<.75 && (xexpect_HCAL-0.187)>-2.015 && (xexpect_HCAL+0.187)<1.285;
	fidu_cut_n = (yexpect_HCAL-0.26)>-.75 && (yexpect_HCAL+0.26)<.75 && (xexpect_HCAL-deltax_shift-0.175)>-2.015 && (xexpect_HCAL+0.175)<1.285;
      }
      else if(SBSM==50){
	deltax_shift=1.10;
	fidu_cut_p = (yexpect_HCAL-0.26)>-.75 && (yexpect_HCAL+0.26)<.75 && (xexpect_HCAL-0.182)>-2.015 && (xexpect_HCAL+0.182)<1.285;
	fidu_cut_n = (yexpect_HCAL-0.26)>-.75 && (yexpect_HCAL+0.26)<.75 && (xexpect_HCAL-deltax_shift-0.164)>-2.015 && (xexpect_HCAL+0.164)<1.285;
      }
      
      if( Wrecon >= Wmin && Wrecon <= Wmax && HCAL_active){
	if( passed_n_cut && fidu_cut_n )
	  {
	    h2_xyHCAL_n_cut->Fill(yHCAL,xHCAL);
	    h2_xyBB_p_exp->Fill(yexpect_HCAL,xexpect_HCAL-deltax_shift);  // xexpect_HCAL-0.65);
	  }
	if( passed_p_cut && fidu_cut_p )
	  {
      ...
      if( Wrecon >= Wmin && Wrecon <= Wmax && (passed_p_cut&&fidu_cut_p) 
	  && (passed_n_cut&&fidu_cut_n) && HCAL_active)
	{
	  h_dxHCAL_cut->Fill( xHCAL - xexpect_HCAL );
	}

      //filling W distributions for n and p
      if( passed_n_cut && fidu_cut_n && HCAL_active){
	h_dpel_n_cut->Fill( dpel );
	h_W_n_cut->Fill( Wrecon );
      }
            
      if( passed_p_cut && fidu_cut_p && HCAL_active){
