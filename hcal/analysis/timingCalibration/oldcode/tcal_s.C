...
void tcal_s( const char *configfilename="stcal_s.cfg", const char *outputfilename="tcal_sout.root"){
  ...
  TH1D *htdcAll = new TH1D("htdcAll","Aggregate TDC (All Channels); ns",800,-400,400);
  TH1D *ht_HCAL_HODO_All = new TH1D("ht_HCAL_HODO_All","TDC (HCal - Hodo, All Channels); ns",800,-400,400);  
  ...
  TH1D *hHCALe = new TH1D( "hHCALe","E HCal Cluster E", 400, 0., 500 );
  ...
  for( Int_t i=0; i<kNcell; i++ ){
    ...
    htdcDiff[i] = new TH1D(Form("htdcDiff_bl%d",i),";TDC_{HCAL}-TDC_{HODO} (ns)",800,-400,400);
    hcorr[i] = new TH1D(Form("hcorr_bl%d",i),"TDC Timewalk Correction; ns",200,0,20);
    htdc_corr[i] = new TH1D(Form("htdc_corr_bl%d",i),";TDC_{HCAL} (ns)",800,-400,400);
    htdcDiff_corr[i] = new TH1D(Form("htdcDiff_corr_bl%d",i),";TDC_{HCAL}-TDC_{HODO} (ns)",800,-400,400);
    he[i] = new TH1D(Form("he_bl%d",i),"a_p; GeV",1000,0,100);
    hatime[i] = new TH1D(Form("hatime_bl%d",i),";ADCt_{HCAL} (ns)",180,0,180);
    htdcVe[i] = new TH2D(Form("htdcVe_bl%d",i),Form(";E_{bl%d} (GeV);TDC_{HCAL} (ns)",i),1000,0.0,500,800,-400,400);
  }
  ...
  for(Int_t i=0; i<kNcell; i++){
    ...
    if( htdcDiff_corr[i]->GetEntries()>tFitMin ){
      htdcDiff_corr[i]->Fit("gaus","Q","",-400,400);
  ...
  cTDC->GetYaxis()->SetLimits(-400,400);
