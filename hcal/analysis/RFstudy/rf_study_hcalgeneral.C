//SSeeds 3.21.23 Script written to investigate the hcal tdc times to hopefully mine out the RF signature from the accelerator
...
void rf_study_hcalgeneral( Int_t kine = 4 ){
  ...
  string configfilename = Form("config/srf_study_gsbs%d.cfg",kine);
  ...
  // Declare outfile
  TFile *fout = new TFile( Form("output/rfStudyOUT_gsbs%d.root",kine), "RECREATE" );
  ...
  // TH2D *hB1B2diffID = new TH2D( "hB1B2diffID", "Hodo Cluster Bar Seed - Next Highest ToT Bar Diff vs ID (uncorrected)", 100, -10, 90, 400, -2, 2 );
  // TH2D *hpvabardiffID = new TH2D( "hpvabardiffID", "Hodo Cluster Bar Seed - Adjacent Bar Diff vs ID (uncorrected)", 100, -10, 90, 400, -2, 2 );
  ...
  TH1D *hhpvablkdiff = new TH1D( "hhpvablkdiff", "HCal Cluster Seed - Adjacent Block Diff", 400, -20, 20 );
  TH2D *hhpvablkdiffID = new TH2D( "hhpvablkdiffID", "HCal Cluster Seed - Adjacent Block Diff vs ID", 288, 0, 288, 400, -20, 20 );
  TH2D *hhpvablkdiffcol = new TH2D( "hhpvablkdiffcol", "HCal Cluster Seed - Adjacent Block Diff vs column", kNcols, 0, kNcols, 400, -20, 20 );
  TH2D *hhpvablkdiffrow = new TH2D( "hhpvablkdiffrow", "HCal Cluster Seed - Adjacent Block Diff vs row", kNrows, 0, kNrows, 400, -20, 20 );
  ...
  for(Long64_t nevent = 0; nevent<Nevents; nevent++){
    ...
    // 	    }
    // 	    if( hcblkid == hpblkid+1 ) hpvabardiffID->Fill( hpblkid, htcbdiff );
    // 	    htbavg += htcbdiff;
    ...
    Int_t pcol = (int)pblkid%kNcols;
    Int_t prow = (int)pblkid/kNcols;
    Double_t blkseed = 0.;
    Double_t tavg = 0;
    ...
    for( Int_t b=0; b<nblk; b++ ){
      ...
      Int_t row = (int)blkid/kNcols;
      Int_t col = (int)blkid%kNcols;
	...
	if( blkid == pblkid+1 ){
	  hhpvablkdiff->Fill( tcdiff );	  
	  hhpvablkdiffID->Fill( pblkid, tcdiff );
	  hhpvablkdiffcol->Fill( pcol, tcdiff );
	  hhpvablkdiffrow->Fill( prow, tcdiff );
	}
	tavg += tcdiff;
