//SSeeds 2.14.23 - Pass2 Update - Calibration code which employs best current cuts on elastic events to obtain ADC gain calibration parameters (pC/GeV). Adds beam energy loss to target and structure updates to easily run on batch farm. Restructured to remove memset and use sbs.hcal.again branch
...
void ecal_hcal( Int_t kine=9, Int_t iter=1 ){
  ...
  while(progress<1.0){
	...
	if( gOldConst[blkid]==0.0 ) gOldConst[blkid]=cblkagain[blk];
	...
	A[blkid] += cblke[blk]/cblkagain[blk];

	if(nblk==1) {
	  A_oneblock[blkid] += cblke[blk]/cblkagain[blk];
	  hpC_vs_ID_oneblock->Fill( blkid, cblke[blk]/cblkagain[blk] );
	}

	hpC->Fill( cluspC );
	hpC_vs_ID->Fill( blkid, cblke[blk]/cblkagain[blk] );
	...
	//Fill some cluster block histograms
  ...
  //   C[x] = x;
  //   Cslice[x] = hpC_vs_ID_oneblock->ProjectionY(Form("pCNslice_%d",x+1),x+1,x+1);
  //   Cslice[x]->Fit("gaus","Q","",0.0,80.0);
  //   f3=Cslice[x]->GetFunction("gaus");
  //   if(Cslice[x]->GetEntries()>0){
  ...
  // TGraphErrors *cpC_onblock_chan = new TGraphErrors( xN, X, Xval, posErr, Xerr );
  // //cEDiff_X->GetXaxis()->SetLimits(HCal_Xi-0.05,HCal_Xf+0.05);  
  // //cEDiff_X->GetYaxis()->SetLimits(0.0,1.0);
  // cEDiff_X->SetTitle("Energy Calibration Check - Dispersive X");
  // cEDiff_X->GetXaxis()->SetTitle("X (m)");
