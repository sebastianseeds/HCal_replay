...
//SSeeds 9.19.22 - Pre-production GEn - Update to previous script. Adds correct paths, small corrections, and cleans up comments.
...
Double_t sampFrac = 0.0795; // sampling frac (0.06588 GeV/0.8286 GeV) = 0.0795 = 7.95% -> (MC E_dep per proton) / (fit to data KE_p)
...
void HCalECal_GEn( const char *configfilename = "setup_HCalECal.cfg", Int_t run = -1 ){
  ...
  Int_t kine = 8; // Keep track of kinematic calibrating from
  Int_t test = 0; // Toggle between official replay and sseeds local replay for loading previous calibration params
  Int_t uni_N = 400; // Total number of bins used to measure detection uniformity (hSampFrac histos)
  ...
  Double_t HCal_div = 0.15; // Transverse width in x and y per cell
  Double_t HCal_Xi = -2.090; // Distance from beam center to top of HCal in analysis coordinates (m)
  Double_t HCal_Xf = 1.360; // Distance from beam center to bottom of HCal in analysis coordinates (m)
  Double_t HCal_Yi = -0.825; // Distance from beam center to opposite-beam side of HCal in analysis coordinates (m)
  Double_t HCal_Yf = 0.825; // Distance from beam center to beam side of HCal in analysis coordinates (m)
  ...
  Double_t HCal_Xmin = HCal_Xi-HCal_div/2;
  Double_t HCal_Xmax = HCal_Xf+HCal_div/2;
  Double_t HCal_Ymin = HCal_Yi-HCal_div/2;
  Double_t HCal_Ymax = HCal_Yf-HCal_div/2;
  ...
  */
  ...
  while(progress<1.0){
	...
	timeremains = timekeeper*( double(Nevents)/double(nevent) - 1. ); 
      ...
      Double_t thetanucleon = acos( (E_e - BBtr_pz[0])/pp ); //use elastic constraInt_t on nucleon kinematics
	...
	Int_t rec_row = ( HCALx - HCal_Xmin )/HCal_div;
	Int_t rec_col = ( HCALy - HCal_Ymin )/HCal_div;
  ...
  //Write out diagnostic histos and prInt_t to console
  //fout->Write();
