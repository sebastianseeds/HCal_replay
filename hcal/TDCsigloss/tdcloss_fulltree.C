//SSeeds 2.13.23 - Short script to run over all data from a given kinematic and check tdc signals with elastic cuts. Configured to include full hcal replays and cosmic data
...
void tdcloss_fulltree( Int_t kine = 4 ){
  ...
  Int_t kineidx = -1;
  if( kine==4 ){
    configfilename = "stdcloss_fulltree_sbs4.cfg";
    kineidx = 0;
  }else if( kine==11 ){
    configfilename = "stdcloss_fulltree_sbs11.cfg";
    kineidx = 1;
  }else{
    cout << "Kinematic not yet configured. Please adjust accordingly." << endl;
    return;
  }
  ...
  while( currentline.ReadLine( configfile ) && !currentline.BeginsWith("#") ){
	...
	TString sval = ( (TObjString*)(*tokens)[1] )->GetString();
        magoff = sval.Atof();
	cout << "LH2 magnet offset: " << magoff << endl;
      }
    }
    delete tokens;
  }
  ...
  Double_t hcalhit_t[maxTracks], hcalhit_id[maxTracks], hcalclusid[maxTracks], hcalclustdc[maxTracks];
  Int_t hcalndata, hcalhit_tndata, hcalhit_idndata;
  ...
  C->SetBranchStatus( "sbs.hcal.hits.t", 1 );
  C->SetBranchStatus( "Ndata.sbs.hcal.hits.t", 1 );  
  C->SetBranchStatus( "sbs.hcal.hits.TDCelemID", 1 );
  C->SetBranchStatus( "Ndata.sbs.hcal.hits.TDCelemID", 1 );
  ...
  C->SetBranchAddress( "sbs.hcal.clus.id", hcalclusid );
  C->SetBranchAddress( "sbs.hcal.clus.tdctime", hcalclustdc );
  ...
  C->SetBranchAddress( "sbs.hcal.hits.t", hcalhit_t );
  C->SetBranchAddress( "Ndata.sbs.hcal.hits.t", &hcalhit_tndata );  
  C->SetBranchAddress( "sbs.hcal.hits.TDCelemID", hcalhit_id );
  C->SetBranchAddress( "Ndata.sbs.hcal.hits.TDCelemID", &hcalhit_idndata );
  ...
  //Double_t magoff[2] = {0.01621,-0.6307}; //Dispersive correction to straight line expectations to account for SBS magnet (in setup file now)
  ...
  TH1D *hNtdc = new TH1D("hNtdc","Total number of HCal TDC hits per event", 150, 0, 150);

  TH2D *hTDChitNvID = new TH2D("hTDChitNvID","HCal TDC Hit Multiplicity vs Channel ID; channel; N hits", 300, -10, 290, 15, 0, 15);
  TH2D *hTDChitNvID_nocut = new TH2D("hTDChitNvID_nocut","HCal TDC Hit Multiplicity vs Channel ID No Cuts; channel; N hits", 300, -10, 290, 15, 0, 15);
  //TH1D *hblkID = new TH1D("hblkID","Primary Block ID", 300, -10, 290);
  ...
  while( progress<1.0 ){
    ...
    while( C->GetEntry( nevent++ ) ){
      ...
      if( nevent%10000==0 ) cout << "Processing Event: " << nevent << " \r";
      cout.flush();
	...
	if(TDCT_id[ihit]==0) hcal_time=TDCT_tdc[ihit];
      ...
      hNtdc->Fill(hcalhit_idndata);


      //Check to verify that missing cluster tdc data cannot be recovered on any other branches where sbs.hcal.hits* has all available tdc data    
      // if( ctdc[0]==-1000 ){
      // 	Double_t ntdchits[maxHCalChan] = {0.};
      // 	bool clushit = false;
      // 	for(Int_t jhit=0; jhit<hcalhit_idndata; jhit++){
      // 	  Int_t hitidx = hcalhit_id[jhit]-1; //Evidently this starts at 1 as well (much to my chagrin)
      // 	  if( hitidx == cid[0]-1 ) {
      // 	    cout << "Missing cluster data on hit branch, event " << nevent << endl;
      // 	    clushit = true;
      // 	  }
      // 	}
      ...
      // 	    cout << cid[b]-1 << " ";
      // 	  }
      // 	  cout << endl << "Cluster PrIDs: ";
      // 	  for(Int_t c=0; c<nclus; c++){
      // 	    cout << hcalclusid[c]-1 << " ";
      // 	  }
      // 	  cout << endl << "Hit IDs: ";
      // 	  for(Int_t jhit=0; jhit<hcalhit_idndata; jhit++){
      // 	    Int_t hitidx = hcalhit_id[jhit]-1;
      // 	    cout << hitidx << " ";
      // 	  }
      // 	  cout << endl << "Hit TDC values on missID: ";
      // 	  for(Int_t jhit=0; jhit<hcalhit_idndata; jhit++){
      // 	    Int_t hitidx = hcalhit_id[jhit]-1;
      // 	    if( hitidx != cid[0]-1 ) continue;
      // 	    Int_t hittdc = hcalhit_t[jhit];
      // 	    cout << hittdc << " ";
      // 	  }
      // 	  cout << endl << "All Chan IDs: ";
      // 	  for(Int_t b=0; b<maxHCalChan; b++){
      // 	    if( hcal_tdc[b]<50000) {
      // 	      cout << b << " ";
      // 	    }
      // 	  }
      // 	  cout << endl << "All Chan TDC value on missID ";
      // 	  cout << hcal_tdc[(int)cid[0]-1] << endl;
	
      // 	}
      // }

      //Crude way to do this, but whatever. Build histo with tdc mult per channel.
      for(Int_t b=0; b<maxHCalChan; b++){
	...
	hTDChitNvID_nocut->Fill( b, Nhits );
      ...
      if(  yexpect_HCAL < (posHCalYf_MC-HCalblk_l_h_MC) &&
	   ...
	   xexpect_HCAL-magoff < (posHCalXf_MC-HCalblk_l_v_MC) &&
	   xexpect_HCAL-magoff > (posHCalXi_MC+HCalblk_l_v_MC)){
	exphcalactive=true;
      }
	...
	continue;
      ...
      //Crude way to do this, but whatever. Build histo with tdc mult per channel after cuts.
      for(Int_t b=0; b<maxHCalChan; b++){
