...
void timingCal( const char *configfilename, int run = -1 ){
  ...
  // Declare outfile
  TFile *fout = new TFile( Form("timingCal_out_%s.root", date.c_str()), "RECREATE" );
  ...
  while(progress<1.0){
	...
	if( fabs(W-W_mean)<W_sig && //Observed mean W cut
	  ...
	  if( HCALe>0.02 && HCALtdc[0]>-400 && nClusHODO<10 ){
	    ...
	    int idx = (int)cblkid[0]; //index of primary block
  ...
  for(int i=0; i<kNcell; i++){
    ...
    if( htDiff[i]->GetEntries()>tFitMin ){
      ...
      //TDCoffsets[i] = -75. - htDiff[i]->GetMean();
      ...
      //cout << "WARNING: Low statistics in cell " << i << ", defaulting to -75. - arithmetic mean for TDC offset:" << -75. - htDiff[i]->GetMean() << "." << endl;
      htDiff[i]->SetName(Form("htDiff_bl%d_r%d_c%d",i,r,c));
    }else{
      //cout << "WARNING: Low statistics in cell " << i << ", defaulting to -75. - PMT type mean for TDC offset:" << -75. - htDiff[i]->GetMean() << "." << endl;
      cout << "WARNING: Low statistics in cell " << i << ", defaulting to old calibration value: " << oldTDCoffsets[i] << "." << endl;
      if(c>3&&c<8){
	//TDCoffsets[i] = -75. - JLAB_mean_TDC;
	TDCoffsets[i] = 0.;
	htDiff[i]->SetName(Form("htDiff_bl%d_r%d_c%d_JLABm%f",i,r,c,JLAB_mean_TDC));	
      }else{
	//TDCoffsets[i] = -75. - CMU_mean_TDC;
	TDCoffsets[i] = 0.;
  ...
  cout << endl << "ADCt Timewalk Corrections: " << endl << endl;
  ...
  */
  ...
  for( int r=0; r<kNrows; r++ ){
    for( int c=0; c<kNcols; c++ ){
      if( htDiff[cell]->GetEntries()<1 ){
	params << "!!" << combTDCoffsets[cell]/TDCCalib << "  ";
	cout << "!!" << combTDCoffsets[cell]/TDCCalib << "  ";
      }else{
	params << combTDCoffsets[cell]/TDCCalib << "  ";
	cout << combTDCoffsets[cell]/TDCCalib << "  ";
      }
