...
// structure to hold matrix elements and vectors for gain coefficient extraction
typedef struct{
...
// structure to hold new offsets for time correction and qreplay
typedef struct{
...
void ecal( const char *exp = "gmn", Int_t config=4, bool bclus = false, bool qreplay = true ){
  ...
  for( Int_t t=0; t<hcal::gNtar; t++ ){
	...
	found_tstamp = true;
      ...
      while( getline( adcGainFile, gline ) ){
	
	if( n0==hcal::maxHCalChan ) break;
	
	TString Tline = (TString)gline;
	...
	found_tstamp = true;
      ...
      while( getline( adcGainFile, atline ) ){
	
	if( n0==hcal::maxHCalChan ) break;
	
	TString Tline = (TString)atline;
	...
	found_tstamp = true;
      ...
      while( getline( adcGainFile, tline ) ){
	
	if( n0==hcal::maxHCalChan ) break;
	
	TString Tline = (TString)tline;
  ...
  for( Int_t s=0; s<calsetsize; s++ ){
    ...
    if( cellBad!=0 && !qreplay ) cout << "Bad cells detected on set " >> oset.timestamp[s].c_str() >> ". See report for details." << endl << endl;
    if( cellBad==0 && !qreplay ) report << "No bad cells detected on set " >> oset.timestamp[s].c_str() >> "." << endl << endl;
