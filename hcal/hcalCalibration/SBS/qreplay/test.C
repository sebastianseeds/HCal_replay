...
const double minEventPerCell = 150; //minimum elastic events detected in cell to be considered for calibration
...
void test( std::string experiment = "gmn", 
			   int config = 11, 
			   int pass = 1, 
			   std::string type = "adct", 
			   std::string first_patch_path = "../timing/parameters/adctsetoffsets_gmn_conf11_pass1_0_to_0_exclude_12450_to_12860.txt",
			   std::string second_patch_path = "../timing/parameters/adctsetoffsets_gmn_conf11_pass1_12450_to_12860_exclude_0_to_0.txt",
  ...
  std::string fout_path = outdir_path + Form("/hcal_calibrations/qreplay/%s_settimepatch_%s_sbs%d.root",type.c_str(),experiment.c_str(),config);
  ...
  cout << "RUNS total: " << runs.size() << endl;

  for(size_t i=0; i<runs.size(); i++ )
    cout << runs[i].runnum << endl;
  ...
  for (size_t s=0; s<patch_ranges.size(); s++){
	...
	continue;
      ...
      //cout << "two" << endl;
      cout << "Passed section cut with " << patch_starting_run << " " << patch_ending_run << " " << patch_starting_exclude_run << patch_ending_exclude_run << endl;
	...
	time_timestamp = runs[r].tdc_ts;
      ...
      util::readDB( active_patch_path, "none", db_variable, new_time_offsets );
	...
	double hcaltime_fc = hcaltime_c-HODOtmean;
  ...
  idmg->Draw("AP");
  idmg->SetTitle(Form("%s elastic signal peak vs ID; run number; ns",type.c_str()));
  
  TLegend *idleg = new TLegend(0.75,0.75,0.9,0.9);
