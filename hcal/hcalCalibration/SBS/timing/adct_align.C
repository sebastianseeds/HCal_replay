...
const Double_t observed_tdc_sigma = 2.0; //rough estimate
...
// Create structure to hold new offsets by timestamp
typedef struct{
  string tstamp;
  vector<Double_t> newtdcoffset;
} TSNOFF;
...
void tdc_align( const char *exp = "gmn", Int_t config=4, bool qreplay = false ){
  ...
  // Set up timestamp index to extract offset by hardware setting
  //vector<string> off_set;

  // Most recent tdc calibration constant for final offsets in ns
  ...
  for( Int_t t=0; t<hcal::gNtar; t++ ){
	...
	delete tokens;
      ...
      //Keep track of relevant timestamp changes for later fits
      // if( off_set.size()==0 ){
      // 	off_set.push_back(allp[t].para[f].tstamp[offidx]);
      // }
      // bool found_duplicate_off = false;
      // bool found_duplicate_cal = false;
      // for( Int_t s=0; s<off_set.size(); s++ ){
      // 	if( off_set[s].compare( allp[t].para[f].tstamp[offidx] )==0 )
      // 	  found_duplicate_off = true;
      // 	if( off_set[s].compare( allp[t].para[f].tstamp[calidx] )==0 )
      // 	  found_duplicate_cal = true;
      // }
      // bool both = false;
      // if( !found_duplicate_off && !found_duplicate_cal )
      // 	both = allp[t].para[f].tstamp[offidx].compare(allp[t].para[f].tstamp[calidx]) != 0;
      // if( !found_duplicate_off || !found_duplicate_cal ){
      // 	if( both ){
      // 	  off_set.push_back( allp[t].para[f].tstamp[offidx] );
      // 	  off_set.push_back( allp[t].para[f].tstamp[calidx] );
      // 	}else if( !found_duplicate_off )
      // 	  off_set.push_back( allp[t].para[f].tstamp[offidx] );
      // 	else if( !found_duplicate_cal )
      // 	  off_set.push_back( allp[t].para[f].tstamp[calidx] );
      // }
      
      // std::vector<Int_t>::iterator it;
      // //Int_t *p;
      // it = std::find( off_set.begin(), off_set.end(), allp[t].para[f].tstamp[offidx] );
      // if( it == off_set.end() ){
      // 	cout << "no duplicates found" << endl;
      // 	off_set.push_back( allp[t].para[f].tstamp[offidx] );
      // }

      // if( std::find(off_set.begin(),off_set.end(),allp[t].para[f].tstamp[offidx]) == off_set.end() )
      // 	off_set.push_back( allp[t].para[f].tstamp[offidx] );
	...
	report << "FOR t " << t << " f " << f << " ADDING timestamp " << oset.timestamp.back();
      ...
      // if( std::find(off_set.begin(),off_set.end(),allp[t].para[f].tstamp[calidx]) != off_set.end() )
      // 	tdccalib_master = ;

      cout << "Parameters loaded for " << hcal::gTar[t] << ", field set " << f << ".." << endl;
	...
	report << endl;
      ...
      cout << "TDC offsets loaded for " << hcal::gTar[t] << ", field set " << f << ".." << endl;

      //Record tdc calibration constant from database
      TString tdcCalibPath = DBpath + "/db_sbs.hcal.dat";
      ifstream tdcCalibFile( tdcOffsetPath );
      report << endl << "Recording tdc calibration constant from database file " << tdcOffsetPath << " with timestamp " << allp[t].para[f].tstamp[calidx] << ".." << endl;
      if( !tdcCalibFile ){
	cerr << endl << "ERROR: No input constant file present -> path to db_sbs.hcal.dat expected." << endl;
	return 0;
      }
	...
	if( found_tstamp && n0==0 && Tline.BeginsWith("sbs.hcal.tdc.calib") ){
      ...
      cout << "TDC calibration constant " << allp[t].para[f].tcalib << " loaded for " << hcal::gTar[t] << ", field set " << f << ".." << endl;
  ...
  for( Int_t set; set<oset.timestamp.size(); set++ ){
  ...
  for( Int_t t=0; t<hcal::gNtar; t++ ){
	...
	for( Int_t l=0; l<runpathstr.length(); l++ ){
	  if( runpathstr[l]=='_' && skipus ){ //read 5 digits from the second underscore
	    for( Int_t d=1; d<=rnct; d++ ){
	      runstr += runpathstr[l+d];
	    }
	    break;
	  }
	  if( runpathstr[l]=='_' && !skipus ){ //skip first underscore
	    skipus=true;
	  }
	} 
	Int_t run = stoi( runstr );

	report << "Analyzing run " << run << ".." << endl;
	cout << "Analyzing run " << run << ".." << endl;

	// Establish set index to build tdc vs ID plots
	Int_t setidx=-1;

	for( Int_t set=0; set<oset.timestamp.size(); set++ ){
	...
	while (C->GetEntry(nevent++)) {
	  ...
	  Double_t tdc_tc = cblktime[0]-HODOtmean; //Primary cluster, primary block tdc with trigger correction (ns)
	  Double_t tdc_tc_rtu = tdc_tc/tdcCalib; //convert back to RTU for alignment over potentially many calibration constants
	  Double_t pblkid = (double)cblkid[0];

	  htp_hodocorr_ID[setidx]->Fill( pblkid, tdc_tc_rtu );
  ...
  // Int_t offsetsize = off_set.size();
  // TCanvas *TDC_top[offsetsize];
  // TCanvas *TDC_bot[offsetsize];
  
  report << "Proceeding to TH2D slices and fits.." << endl;

  // Declare canvas and graph for plots
  Int_t offsetsize = oset.timestamp.size();
  TCanvas *TDC_top[offsetsize];
  TCanvas *TDC_bot[offsetsize];
  TGraphErrors *gtdc_c[offsetsize];

  // Output text file for new TDC offsets
  ofstream writeParFile( writeParPath );
  writeParFile.open( writeParPath );

  //for( Int_t set; set<off_set.size(); set++ ){
  for( Int_t set; set<oset.timestamp.size(); set++ ){
    ...
    Double_t tcell[hcal::maxHCalChan] = {0.};
    Double_t tcval[hcal::maxHCalChan] = {0.};
    Double_t tcvalw[hcal::maxHCalChan] = {0.};
    Double_t tcerr[hcal::maxHCalChan] = {0.};
    Double_t tcerrw[hcal::maxHCalChan] = {0.};
    TH1D *tcellslice[hcal::maxHCalChan];
    
    //temp
    Double_t oldTDCoffsets[hcal::maxHCalChan] = {0.};
    //Double_t TDCCalib = allp[0].para[0].tcalib;
    Double_t calib_target = tdccalib_master;
    Double_t TDC_target = -75.;
	...
	TDC_bot[set]->cd(c+1-half_chan);
      ...
      Double_t tfith = 0.;
      tcell[c] = c;
      tcellslice[c] = htp_hodocorr_ID[set]->ProjectionY(Form("tcellslice_%d",c+1),c+1,c+1);
      oldTDCoffsets[c] = allp[ti].para[fi].otdcoff[c]; //TEMP for testing
      tcval[c] = oldTDCoffsets[c]; 
      tcvalw[c] = 0; 

      Int_t sliceN = tcellslice[c]->GetEntries();
      if( sliceN<fit_event_min ){
	tcellslice[c]->Draw();
	continue;
      }
      ...
      tsetpar[0] = sliceN;
      tsetpar[1] = arimean;
      tsetpar[2] = observed_tdc_sigma;
      tfitl = arimean - 3*observed_tdc_sigma;
      tfith = arimean + 3*observed_tdc_sigma;
      TF1 *gausfit = new TF1("gausfit",hcal::g_gfit,lower_lim,upper_lim,3);
      gausfit->SetLineWidth(4);
      gausfit->SetParameter(0,tsetpar[0]);
      gausfit->SetParameter(1,tsetpar[1]);
      gausfit->SetParLimits(1,lower_lim,upper_lim);
      ...
      tcvalw[c] = gausfit->GetParameter(1)*calib_target;
      tcerr[c] = gausfit->GetParameter(2);
      tcerr[c] = gausfit->GetParameter(2)*calib_target;
    ...
    //TGraphErrors *gtdc_c = new TGraphErrors( hcal::maxHCalChan, tcell, tcvalw, cellerr, tcerr );
    gtdc_c[set] = new TGraphErrors( hcal::maxHCalChan, tcell, tcvalw, cellerr, tcerr );
    gtdc_c[set]->GetXaxis()->SetLimits(-10,290);  
    gtdc_c[set]->GetYaxis()->SetLimits(lower_lim,upper_lim);
    gtdc_c[set]->SetTitle(Form("TDC_{hcal}-TDCmean_{hodo} vs Cell, timestamp: %s",ts.c_str() ) );
    ...
    for( Int_t i=0; i<hcal::maxHCalChan; i++ ){
      if( oldTDCoffsets[i]>0 && ( tcval[i]==0 || abs(tcerr[i])<0.05 ) ){ //This will only work if most channels are closely aligned already and oldtdcoffsets aren't believable
	writeParFile << tcval_avg << endl;
	report << tcval_avg << " with adjustment.." << endl;
      }else{
	//writeParFile << tcval[i]/calib_target + oldTDCoffsets[i] - TDC_target/calib_target << endl;
	//report << tcval[i]/calib_target + oldTDCoffsets[i] - TDC_target/calib_target << endl;
	writeParFile << tcval[i] + oldTDCoffsets[i] - TDC_target/calib_target << endl;
	report << tcval[i] + oldTDCoffsets[i] - TDC_target/calib_target << endl;
      }
    }  
    report << endl << endl;
  ...
  // writeParFile << "#HCal TDC offsets obtained " << date.c_str() << endl;
  // writeParFile << "#Offsets obtained from fits over TDC distributions = " << endl;
  // report << endl << endl << "#HCal TDC offsets obtained " << date.c_str() << endl;
  // report << "#Offsets obtained from fits over TDC distributions = " << endl;
