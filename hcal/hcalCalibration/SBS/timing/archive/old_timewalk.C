...
const Double_t trad_p1_llim = 0.05;
const Double_t trad_p1_ulim = 4.;
...
void timewalk( const char *experiment = "gmn", Int_t config = 7, bool qreplay = false, Int_t pass = 0 ){
  ...
  for (Int_t r=0; r<nruns; r++) {
	...
	xyhcalexp[0] < hcal::posHCalXi;
      ...
      Double_t pblke = cblke[0]/tw_cal[Ncal_set_idx].old_param[(Int_t)pblkid]*new_adcg_coeff[(Int_t)pblkid]; //corrected with new gain parameters

      //ADC time 
      Double_t pblkatime = cblkatime[0];
      //align atime with new params, paramC is adct offset
      Double_t hcaladct = pblkatime+tw_cal[Ncal_set_idx].old_paramC[(Int_t)pblkid]-new_adct_offsets[(Int_t)pblkid]; 
	...
	tw_cal[Ncal_set_idx].old_paramB[(Int_t)pblkid] * tw_cal[Ncal_set_idx].tdc_calib - 
	new_tdc_offsets[(Int_t)pblkid] * tw_cal[Ncal_set_idx].tdc_calib;
  ...
  cout << "Parameters written to file. Writing output root file to " << tw_rootpath << ".." << endl;
