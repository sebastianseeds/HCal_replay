//SSeeds 8.15.23 Calibration script which parses back tdc_tw.C to remove all calibrations by channel, but adds linear timewalk fits for comparison by kinematic and also extracts ADC time timewalk pol1 fit parameters
...
//In order to override the timewalk parameters from a file different than the new_tw_path set by config and experiment, pass the path instead of false for string param_override
void tw_gen( const char *experiment = "gmn", Int_t config = 7, bool qreplay = false, Int_t pass = 0, std::string param_override = "false", std::string param_ts = "null" ){
  ...
  std::string tw_path = outdir_path + Form("/hcal_calibrations/pass%d/timing/tw_general_%s_conf%d_qr%d_pass%d.root",pass,experiment,config,(Int_t)qreplay,pass);
  ...
  std::string new_tw_path = Form("../timing/parameters/tw_general_%s_conf%d_pass%d.txt",experiment,config,pass);
  ...
  cout << "beginning loop over runs.." << endl;

  //Main loop over runs
  for (Int_t r=0; r<nruns; r++) {

    cout << "beginning loop over run idx " << r << endl;
    ...
    cout << "moving to target parameters.. " << endl;
    ...
    cout << "moving to getting paths.." << endl;
    ...
    cout << rootfile_dir << " " << current_runnumber << endl;

    std::string rootfile_path = rootfile_dir + Form("*%d*",current_runnumber);

    cout << rootfile_path << endl;

    cout << "got run parameters for run number " << current_runnumber << ", checking target, and printing details on change.." << endl;
    ...
    cout << "getting timestamp, setting up arrays and vectors to hold offsets and tw params, and filling with set by timestamp.." << endl;
    ...
    cout << "tdc active timestamp " << tdc_active_timestamp << endl;
    ...
    cout << "adct active timestamp " << adct_active_timestamp << endl;
    ...
    cout << "adc gain active timestamp " << active_timestamp << endl;
    
    cout << "if qreplay, reading in recent tw calibration parameters.." << endl;
    ...
    }else if( qreplay ){ //if param_override is true, get the tw parameters from a different path
      util::readDB( param_override, param_ts, db_tdctw_trad_variable, new_tdctw_tradpar );  
      util::readDB( param_override, param_ts, db_tdctw_expo_variable, new_tdctw_expopar );  
      util::readDB( param_override, param_ts, db_tdctw_pol_variable, new_tdctw_polpar );  
      util::readDB( param_override, param_ts, db_adcttw_pol_variable, new_adcttw_polpar );  
    }

    //////////
    //Select correct calibration set. Timewalk corrections will go with energy, active_timestamp left unaltered.
    cout << "selecting calibration set with active timestamps.." << endl;
    ...
    cout << "getting cuts for this set.." << endl;
    ...
    cout << "setting up chain and loading: " << rootfile_path << endl;

    // Setting up chain and branch addresses
    C = new TChain("T");
    C->Add(rootfile_path.c_str());

    cout << "linking tree variables.." << endl;
    ...
    cout << "setting up hcal coordinate axes and declaring event vars.." << endl;
    ...
    cout << endl << "NEVENTS:" << nevents << endl;
    
    //Main loop over events in run
    while(C->GetEntry(nevent++)) {

      //cout << endl << "analyzing run: " <<  current_runnumber << ": " <<  nevent << "/" << nevents << endl;

      //reporting
      // cout << "Analyzing run " << current_runnumber << ": " <<  nevent << "/" << nevents << " \r";
      // cout.flush();

      //Lead with elastic cuts to constrain timing in sequence of descending complexity (to improve processing time).
	...
	GlobalCut->UpdateFormulaLeaves();
      ...
      cout << "calculating adct times.." << endl;

      ///////
      //HCal coincidence time cut (using adctime while hcal tdc suspect, new offsets)
      //Removed this cut when calibrating adc time timewalk
      //define primary block, primary cluster ID
      Int_t pblkid = cblkid[0]-1; 
      //offset corrected adc time 
      Double_t hcaladct = cblkatime[0]+tw_cal[Ncal_set_idx].old_paramC[pblkid]-new_adct_offsets[pblkid]; //new atime, paramC is adct offset
      //correct the adc time for trigger jitter with hodoscope cluster mean time
      Double_t hcaladct_hodo = hcaladct - HODOtmean; //hodoscope corrected adc time
      ...
      bool failedcoin = abs(hcaladct-atime0)>adct_Nsig*atimesig;
	  
      //if( failedcoin ) continue; //All events where adctime outside of reasonable window cut

      cout << "many physics calculations.." << endl;

      ///////
      //Physics calculations for additional elastic cuts
      ...
      cout << "cutting on W2 and acceptance.." << endl;
	...
	xyhcalexp[0] < hcal::posHCalXi;
      ...
      //////
      //Log bad tdc and keep track of total missing primary cluster events
      //do not cut on this when calibrating adc time
      ...
      cout << "calculating tdc times.." << endl;

      ///////
      //Address timewalk with primary cluster primary block tdc time vs energy and adct vs energy
      //primary block energy
	...
	new_tdc_offsets[pblkid] * tw_cal[Ncal_set_idx].tdc_calib;
      ...
      cout << "filling qreplay histograms.." << endl;
	...
	hadct_twpol[Ncal_set_idx]->Fill( hcaladct_tw_pol );
      ...
      cout << "filling gen histograms.." << endl;
    ...
    cout << "ending runnumber " << current_runnumber << " right before reset" << endl;
  ...
  for( Int_t s=0; s<Ncal_set_size; s++ ){
    ...
    adcttw_pol[s] = new TCanvas(Form("TDC_linearfit_s%d",s),Form("TDC_linearfit_s%d",s),1600,1200);
    ...
    TF1 *fittdc_pol = new TF1( "fittdc_pol", util::g_lfit, fit_ll, fit_ul );
    ...
    TF1 *adcttestfit = new TF1( "adcttestfit", "pol1" );
    //TF1 *fitadct_pol = new TF1( "fittdc_pol", util::g_lfit, fit_ll, fit_ul );
