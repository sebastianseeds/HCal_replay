//seeds 10.1.23 Test tmax on MC elastic, hcal nblk
...
const double ecut = 0.005; //replicate default values from db_sbs.hcal.dat
...
void test_tmax_mc(int sbs = 11){//main
 
  TChain *C = new TChain("T");

  string rootfilepath = Form("/lustre19/expphy/volatile/halla/sbs/seeds/sim091523/replayed_gmn_sbs%d*.root",sbs);

  string outputfilename = Form("root_out/tmaxcompMCOUT_sbs%d.root",sbs);
  ...
  double hcalB_id[maxPB], cblkatime[maxPB], cblke[maxPB];
  ...
  TH1D *ht_all_diff_30ns = new TH1D("ht_all_diff_30ns",
			       "adct all channels/runs all cluster block difference, tmax=30ns",
			       total_bins_short,
			       lower_lim_short,
			       upper_lim_short);


  TH1D *ht_all_diff_9ns = new TH1D("ht_all_diff_9ns",
			       "adct all channels/runs all cluster block difference, tmax=9ns",
			       total_bins_short,
			       lower_lim_short,
			       upper_lim_short);
  ...
  TH1D *hnblk_diffcut_30ns = new TH1D("hnblk_diffcut_30ns", " ;N", 20,0,20);


  TH1D *hnblk_diffcut_9ns = new TH1D("hnblk_diffcut_9ns", " ;N", 20,0,20);
  ...
  for( Long64_t nevent = 1; nevent <Nevents; nevent++){
    ...
    //replicate sbs.hcal.emin_clSeed
    if( pblke<=ecut )
      continue;
    ...
    //Start markers at 1 to include pblk and increment
    int nblk_30ns = 1;
    int nblk_9ns = 1;
	...
	continue;
      ...
      if( abs(diff)<=9. ){
	nblk_9ns++;
	ht_all_diff_9ns->Fill(diff);
      }
      if( abs(diff)<=30. ){
	nblk_30ns++;
      	ht_all_diff_30ns->Fill(diff);
      }
	...
	ht_all_diff_far->Fill(diff);
      ...
      if( !failedaccmatch && !failedglobal )
    ...
    //Fill nblk histos
    hnblk->Fill(nblk);
    hnblk_diffcut_30ns->Fill(nblk_30ns);
    hnblk_diffcut_9ns->Fill(nblk_9ns);
