...
void tcal( Int_t kine=-1, Int_t iter=0 ){
  ...
  string RtwP0path = Form("parameters/twP0_sbs%d.txt",kine);
  string RtwP1path = Form("parameters/twP1_sbs%d.txt",kine);
  string RtwP2path = Form("parameters/twP2_sbs%d.txt",kine);
  ...
  if( qreplay ){
    ...
    ifstream twP0file( RtwP0path );
    if( !twP0file ){
      cerr << endl << "ERROR: No input constant file present -> path to twP0.txt expected." << endl;
      return 0;
    }
    ...
    ifstream twP1file( RtwP1path );
    if( !twP1file ){
      cerr << endl << "ERROR: No input constant file present -> path to twP1.txt expected." << endl;
      return 0;
    }
    ...
    ifstream twP2file( RtwP2path );
    if( !twP2file ){
      cerr << endl << "ERROR: No input constant file present -> path to twP2.txt expected." << endl;
      return 0;
    }
    ...
    while( getline( newtdcfile, line4 ) ){
      if( line4.at(0) == '#' ) {
	continue;
      }
    ...
    while( getline( newadctfile, line5 ) ){
      if( line5.at(0) == '#' ) {
	continue;
      }
      stringstream ss( line3 );
 ...
 // Reading all relevant config files for ld2
  ...
  TH1D *hadct_corr = new TH1D("hadct_corr","ADCt Hodo Corrected (All Channels); ns",atotb,allim,aulim);
  ...
  for( Int_t f=0; f<nfset_lh2[kIdx]; f++ ){
    ...
    for( Long64_t nevent = 1; nevent <Nevents_h[f]; nevent++){
      ...
      htp_ID->Fill( int(cblkid_h[f][0])-1, cblktime_h[f][0] );
      hap_ID->Fill( int(cblkid_h[f][0])-1, cblkatime_h[f][0] );
	...
	if( qreplay ) TDCcorr = oldP0[blkid]*exp(-oldP1[blkid]*blke);
	...
	if( qreplay ){
	  ...
	  if( blkatime_new>0 ){
	    ...
	    hadct_corr->Fill( blkatime_new - hodot );
	    ha_ID->Fill( blkid, blkatime_new );
	    haDiff_ID->Fill( blkid, blkatime_new - hodot );
	  }
	  if( blktime_new<10000 && blktime_new>-500){
  ...
  for( Int_t f=0; f<nfset_ld2[kIdx]; f++ ){
    ...
    for( Long64_t nevent = 1; nevent <Nevents_d[f]; nevent++){
      ...
      //Fill some histograms with only the primary block timing
      htp_ID->Fill( int(cblkid_d[f][0])-1, cblktime_d[f][0] );
      hap_ID->Fill( int(cblkid_d[f][0])-1, cblkatime_d[f][0] );
	...
	if( qreplay ){
	  ...
	  if( blktime<10000 && blktime>-500){
	    ...
	    htDiff_ID->Fill( blkid, blktime - hodot );
	    htCorr_ID->Fill( blkid, blktime - hodot - TDCcorr ); //Will only effect the dist on iter 1
	    htdcVe[blkid]->Fill( blke, blktime );
	  }
	}
	NEV[blkid]++;
	TNEV_d++;
  ...
  for(Int_t c=0; c<kNcell; c++){
    ...
    if( htdcVe[c]->GetEntries()>tFitMin ){
      htdcVe[c]->Fit("fitTW","Q","",5,300);
      TvseP0[c] = fitTW->GetParameter(0);
      TvseP1[c] = fitTW->GetParameter(1);
      TvseP2[c] = fitTW->GetParameter(2);
      htdcVe[c]->SetTitle(Form("P0:%f P1:%f P2:%f",TvseP0[c],TvseP1[c],TvseP2[c]));
    }
