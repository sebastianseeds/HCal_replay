...
void ecal_hcal_pass0( Int_t kine=4, Int_t iter=1 ){
  
  if( kine!=4 && kine!=7 ){
    cout << "This script depricated after pass0. Please use ecal_hcal.C." << endl;
    return;
  }
  ...
  // Declare outfile
  //TFile *fout = new TFile( "eCalEOut_pass0_test.root", "RECREATE" );
  TFile *fout = new TFile( Form("eCalEOut_SBS%d.root",kine), "RECREATE" );
...
//TH1D *hW2_coincut = new TH1D( "W2_coincut", "W2 Coin Cut; GeV", 250, 0.3, 1.5 );
  ...
  while(progress<1.0){
	...
	timeremains = timekeeper*( double(Nevents)/double(nevent) - 1. ); 
	  ...
	  crow==23 || 
	  ccol==0 || 
	  ccol==11 ) continue; //All events with primary cluster element on edge blocks cut
      if( abs(cblkatime[0]-atime0)>3*atime_sig ) continue; //All events where adctime outside of reasonable window cut
      //////////////////////////////////////////////////////////////////////
      ...
      //////////////////////////////////////////////////////////////////
      //Cut on dy. Done here to preserve diagnostic histograms
	...
	A[blkid] += cblke[blk]/gOldConst[blkid];
  	  
	if(nblk==1) {
	  A_oneblock[blkid] += cblke[blk]/gOldConst[blkid];
	  hpC_vs_ID_oneblock->Fill( blkid, cblke[blk]/gOldConst[blkid] );
	  hsclusblkE->Fill(cblke[blk]);
	  hCoeffvID_oneblock->Fill( blkid, E_exp/cluspC );
	}
	...
	hpC->Fill( cluspC );
	hpC_vs_ID->Fill( blkid, cblke[blk]/gOldConst[blkid] );
  ...
  for(Int_t i=0; i<kNcell; i++){
    ...
    coefficients_1b->Fill(i,GCoeff_oneblock[i]);
  }
...
\
  //Construct graphs for one block analysis
  TF1 *gausfit = new TF1("gausfit",Gfit,0.0,0.3,3);
  gausfit->SetLineColor(kMagenta);
  gausfit->SetParameter(0,4);
  gausfit->SetParameter(1,0.005);
  gausfit->SetParLimits(1,0.0,0.3);
  gausfit->SetParameter(2,0.005);
  Double_t posCErr[kNcell] = {0.};
  //For X (dispersive)
  Double_t Ch[kNcell];
  Double_t Cval[kNcell];
  Double_t Cerr[kNcell];
  TH1D *Cslice[kNcell];

  //Across all channels
  for( Int_t x=0; x<kNcell; x++ ){
    Ch[x] = x;
    Cslice[x] = hCoeffvID_oneblock->ProjectionY(Form("pCNslice_%d",x+1),x+1,x+1);
    Cslice[x]->Fit("gausfit","QRB+");
    Cslice[x]->Write();
    //f3=Cslice[x]->GetFunction("gaus");
    if(Cslice[x]->GetEntries()>30){
      //Cval[x] = f2->GetParameter(1);
      //Cerr[x] = f2->GetParameter(2);
      Cval[x] = gausfit->GetParameter(1);
      Cerr[x] = gausfit->GetParameter(2);
    }else{
      Cval[x] = 0.;
      Cerr[x] = 0.;
      //cout << "Warning: Not enough statistics at channel " << x << " for one block analysis. Using old gain coefficent here." << endl;
    }
    cout << Ch[x] << " " << Cval[x] << endl;
  }
  //Calculate the gain coefficient with sampling fraction from MC
  

  TGraphErrors *cCoeff_ch = new TGraphErrors( kNcell, Ch, Cval, posCErr, Cerr );
  //cCoeff_ch->GetXaxis()->SetLimits(HCal_Xi-0.05,HCal_Xf+0.05);  
  //cCoeff_ch->GetYaxis()->SetLimits(0.0,1.0);
  cCoeff_ch->SetTitle("One Block Coefficients");
  cCoeff_ch->GetXaxis()->SetTitle("Channel");
  cCoeff_ch->GetYaxis()->SetTitle("GeV/pC");
  cCoeff_ch->SetMarkerStyle(20); // idx 20 Circles, idx 21 Boxes
  cCoeff_ch->Write("cCoeff_ch");
