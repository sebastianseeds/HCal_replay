...
void ecal_hcal( Int_t kine=9, Int_t iter=1 ){
  ...
  string configfilename = Form("../config/GMn/SBS%d/secal_sbs%d.cfg",kine,kine);
  ...
  C->SetBranchAddress( "sbs.hcal.clus_blk.again", cblkagain ); // Array of block ADC times
  ...
  // Declare outfile
  TFile *fout = new TFile( "eCalEOut_test.root", "RECREATE" );
  
  // Initialize vectors and arrays
  Double_t gRatio[kNcell] = {0.0}; //Old ratios from first iteration
  Double_t oneBlock[kNcell] = {0.0};
  Double_t GCoeff[kNcell] = {0.0};
  Double_t GCoefff[kNcell] = {0.0};
  Double_t GCoeff_2[kNcell] = {0.0};
  Double_t GCoeff_oneblock[kNcell] = {0.0};
  Double_t GCoeff_divide[kNcell] = {0.0};
  Double_t AA[kNcell] = {0.0}; // Array to keep track of ADC values per cell. Memory is reset on each event.
...
//TH1D *hW2_coincut = new TH1D( "W2_coincut", "W2 Coin Cut; GeV", 250, 0.3, 1.5 );
  ...
  while(progress<1.0){
	...
	timeremains = timekeeper*( double(Nevents)/double(nevent) - 1. ); 
      ...
      Double_t A_2[kNcell] = {0.0}; // Array to keep track of ADC values per cell. Outscope on each ev
      Double_t A_oneblock[kNcell] = {0.0}; // Array to keep track of ADC values per cell. Outscope on each ev
      
      memset(AA, 0, kNcell*sizeof(double));
	...
	if( gOldConst_2[blkid]==0. ) gOldConst_2[blkid]=cblkagain[blk];

	if( gOldConst[blkid] != cblkagain[blk] ) cout << "adc gain tree vs DB mismatch on " << blkid << endl;
	...
	clusE += cblke[blk];
	clusE_2 += cblke[blk]/cblkagain[blk]; 
	A[blkid] += cblke[blk];
	AA[blkid] += cblke[blk];
	A_2[blkid] += cblke[blk]/cblkagain[blk];
	if(nblk==1) A_oneblock[blkid] += cblke[blk];
  	  
	//Error per event (2.1.23 estimate)
	//cluspC = cblke[blk]/gOldConst[blkid];
	cluspC = cblke[blk]/cblkagain[blk];
	...
	if(nblk==1){
	  //ba_err(icol)+=E_exp;
	  //bb(icol)+= A[icol];
	  ba_oneblock(icol)+= A_oneblock[icol];
	  bb(icol)+= E_exp;
	...
	for(Int_t irow = 0; irow<kNcell; irow++){
	  ...
	  Maa(icol,irow) += AA[icol]*AA[irow]/E_exp;
	  Ma_2(icol,irow) += A_2[icol]*A_2[irow]/E_exp;
  ...
  */
  ...
  for(Int_t i=0; i<kNcell; i++){
    ...
    //Do not change ADC gain coeff if insufficient events or energy dep in cell
    if( NEV[i] < minEventPerCell || Maa(i,i) < 0.1*baa(i) ){ 

      cellBad = 1;

      Double_t elemRatio = Maa(i,i)/baa(i);

      baa(i) = 1.0;  // Set RHS vector for cell i to 1.0 
      Maa(i,i) = 1.0; // Set diagonal element of Matrix M for cell i to 1.0 
    ...
    //Do not change ADC gain coeff if insufficient events or energy dep in cell, oneblock alt
	...
	if( j != i ){
      ...
      badcell_oneblockalt[i] = 1;
      cout << "oneblock alt analysis" << endl;
      cout << "cell" << i << " bad" << endl;
      cout << "Number of events in bad cell =" << NEV[i] << endl;
      cout << "Matrix element/vector element ratio =" << elemRatio << endl;
    } 
    ...
    //Do not change ADC gain coeff if insufficient events or energy dep in cell
    if( NEV[i] < minEventPerCell || Ma_2(i,i) < 0.1*ba_2(i) ){ 

      cellBad = 1;

      Double_t elemRatio = Ma_2(i,i)/ba_2(i);

      ba_2(i) = 1.0;  // Set RHS vector for cell i to 1.0 
      Ma_2(i,i) = 1.0; // Set diagonal element of Matrix M for cell i to 1.0 
  ...
  */
  ...
  TMatrixD MM_inv = Maa.Invert();
  TMatrixD M_inv_2 = Ma_2.Invert();
  ...
  TVectorD Coefff = MM_inv*baa; // Stays unmodified for reference
  TVectorD Coeff_2 = M_inv_2*ba_2; // Stays unmodified for reference
  ...
  for( Int_t i=0; i<kNcell; i++ ){
    if(badcell_oneblock[i]==0){
      ...
      oneBlockCoeff[i] = gOldConst[i]*(ba[i]/oneBlock[i]);
  ...
  for(Int_t i=0; i<kNcell; i++){
    if(badcell[i]==0){
      GCoefff[i]=gOldConst[i]*Coefff[i];
      ...
      GCoeff_2[i]=Coeff_2[i]; // The second version here uses the sbs.hcal.again branch to eliminat the ratio
      GCoeff_divide[i]=GCoeff[i]/oneBlockCoeff[i];

    }else{
      GCoeff[i]=gOldConst[i]; // If the cell is bad, use the old coefficient
      GCoefff[i]=gOldConst[i]; // If the cell is bad, use the old coefficient
      GCoeff_2[i]=gOldConst_2[i];
      GCoeff_divide[i]=-1.0;
    }
  }
...
//  cout << endl;
  ...
  */
  ...
  cell = 0;
  cout << "Gain Coefficient test" << endl;
  for( Int_t r=0; r<kNrows; r++ ){
    for( Int_t c=0; c<kNcols; c++ ){
      cout << GCoeff[cell] << " - " << GCoeff_2[cell] << " = " << GCoeff[cell]-GCoeff_2[cell] << "  ";
      cell++;
    }
    cout << endl;
  }
