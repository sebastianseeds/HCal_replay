...
//
void replay_bbcosmics(int run_number = 124, uint nev = -1, TString start_name = "e1209019",Bool_t samps=kFALSE)
{
  uint nseg = 0;
//load SBS-offline
  ...
  ts->SetDataOutputLevel(1);
  if (samps) ts ->SetStoreRawHits(kTRUE);
  ts ->SetStoreEmptyElements(kFALSE);
   bigbite->AddDetector( ts );

  SBSGenericDetector* tdctrig= new SBSGenericDetector("tdctrig","BigBite shower ADC trig");
  tdctrig->SetModeADC(SBSModeADC::kNone);
  tdctrig->SetModeTDC(SBSModeTDC::kTDC);
  bigbite->AddDetector( tdctrig );

  /* SBSGenericDetector* hcaltrig= new SBSGenericDetector("hcaltrig","BigBite shower ADC trig");
  hcaltrig->SetModeADC(SBSModeADC::kNone);
  hcaltrig->SetModeTDC(SBSModeTDC::kTDCSimple);
  bigbite->AddDetector( hcaltrig );
  */
 ...
 bbtrig->SetStoreRawHits(kTRUE);
  ...
  while(seg_ok && seg < max_seg) {
    ...
    //start_name = "e1209019_trigtest";
     ...
     //data_fname = TString::Format("%s_%d.evio.%d",start_name.Data(),run_number,seg);
    ...
    //   for (Int_t nn=0;nn<pathList.size();nn++) {
    //    TString data_fname_all = TString::Format("%s/%s_%d.evio.%d",pathList[nn].Data(),start_name.Data(),run_number,seg);
    ...
    analyzer->SetCrateMapFileName( "cratemapHere" );
