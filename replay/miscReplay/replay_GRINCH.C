...
//void replay_GRINCH(Int_t runnum = 10080, Int_t lastEvent = -1){
void replay_GRINCH(Int_t runnum = 32, Int_t lastEvent = -1){
  ...
  grinch_adc->SetModeADC(SBSModeADC::kADCSimple);
  ...
  SBSEArm *bb = new SBSEArm("bb","EArm");
...
//  THaRun* run = new THaRun(TString::Format("/cache/halla/sbs/raw/e1209019_trigtest_%d.evio.0",runnum));
...
//  run->SetDataRequired(7);
  ...
  analyzer->SetOutFile( TString::Format("../Rootfiles/mkj_grinch_%d.evio.0.root",runnum));
