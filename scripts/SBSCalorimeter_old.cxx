...
Int_t SBSCalorimeter::FindClusters()
{
  ...
  while ( NSize != 0 )  {
	...
	Bool_t InTime=kFALSE; //Bool to track check on block timing coincidence
	Bool_t IsNeighbor=kFALSE; //Bool to track check on block proximity
	...
	while (!IsNeighbor && (it2 < fBlockSet.end())) {
	  ...
	  if (IsNeighbor) {
	    ...
	    InTime=( fabs( blk->GetAtime()-pTime ) < fTmax);
	    if(InTime) {	
	      fClusters[Index]->AddElement(blk);
	    }
	  } else {	       
